...
class Idle(Node):
    """Всегда удерживает дрон в позиции и публикует команды"""
    def __init__(self):
        super().__init__('idle')
        ...
        self.kp_xy = 0.5
        self.kp_z = 0.5
        self.timer = self.create_timer(1.0/20.0, self._hold_position)  # 20 Hz
        self.active = True
    ...
    def _hold_position(self):
        if self.current_pos is None or self.current_imu is None or self.target_pos is None:
            return
...
class Planner(Node):
    def __init__(self, idle_node, ISGPS=0):
        super().__init__('planner')
        self.idle = idle_node
        self.pub = self.idle.pub  # используем тот же паблишер для AttitudeTarget
    ...
    def set_gps_mode(self):
        client = self.create_client(SetMode, '/uav1/set_mode')
        ...
        if future.result():
            self.get_logger().info("SetMode GUIDED_NOGPS applied")

    def pos_callback(self, msg):
        self.current_pos = msg.pose.position

    def plan_trajectory(self, start, goal, v_max=0.3, a_max=0.2, dt=0.1):
        """Сниженная скорость для плавного перемещения"""
    ...
    def _step_trajectory(self):
        ...
        self.pub.publish(msg)
        # обновляем цель для Idle чтобы она была корректна
        self.idle.set_target(self._goal_pos)

    def execute_callback(self, goal_handle):
        self.get_logger().info("NavigateToPose goal received, disabling Planner in Idle")
        ...
        goal_handle.succeed()
        self.get_logger().info("Trajectory loaded, Idle target updated")
        return NavigateToPose.Result()

def main(args=None):
    rclpy.init(args=args)
    idle_node = Idle()
    planner_node = Planner(idle_node, ISGPS=1)
    try:
